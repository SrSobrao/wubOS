(function ($) {
	...
	loadYoumax = function() {	
		...
		$(this).append('<div id="youmax-encloser"><iframe id="youmax-video" width="'+(youmaxWidgetWidth-2)+'" height="'+(youmaxWidgetWidth/this.youmax_global_options.youtubeVideoAspectRatio)+'" src="" frameborder="0" allowfullscreen></iframe><div id="youmax-video-list-div"></div><div id="youmax-load-more-div">Load More</div></div>');
		
		$(this).find('#youmax-video').hide();
		
		$(this).append('<div id="youmax-lightbox"><div style="width:100%; position:absolute; top:20%;"><iframe id="youmax-video-lightbox" width="640" height="360" src="" frameborder="0" allowfullscreen></iframe></div></div>');
		
		$(this).find('#youmax-lightbox').hide();
	},
	...
	showLoader = function() {
		...
		$(this).find('#youmax-video-list-div').append('<div style="text-align:center; height:200px; font:14px Calibri;"><br><br><br><br><br><br>loading...</div>');
	},
	
	initFeaturedVideos = function () {
		youTubePlaylistURL = this.youmax_global_options.youTubePlaylistURL;
		console.log('inside init featured - '+youTubePlaylistURL);
		if(null!=youTubePlaylistURL&&youTubePlaylistURL.indexOf("youtube.com/playlist?list=")!=-1) {
			youmaxFeaturedPlaylistId = youTubePlaylistURL.substring(youTubePlaylistURL.indexOf("?list=")+6);
			this.youmax_global_options.youmaxFeaturedPlaylistId = youmaxFeaturedPlaylistId;
		}
	},
	...
	showPlaylists = function(response,loadMoreFlag) {
		...
		var self = this;
		$(this).find('.youmax-video-tnail-box').click(function() {
			//alert(this.id);
			showLoader.call(self);
			playlistTitle = $(this).find(".youmax-video-list-title").text();
			getUploads.call(self, "play_"+this.id,playlistTitle);
	...
	showUploads = function(response,playlistTitle,loadMoreFlag) {
		...
		if(!loadMoreFlag) {
			...
			if(playlistTitle) {
				...
				$(this).find('#youmax-video-list-div').append('<span class="youmax-showing-title youmax-tab-hover" id="uploads_'+response.items[0].snippet.playlistId+'" style="max-width:100%;"><span class="youmax-showing">&nbsp;&nbsp;Showing playlist: </span>'+playlistTitle+'</span><br/>');
			}
		}
		...
		for(var i=0; i<uploadsArray.length; i++) {
			...
			if((i+this.youmax_global_options.youmaxItemCount)%youmaxColumns!=0)
		...
		$(this).find('.youmax-video-tnail-box').click(function(){
			...
			if(self.youmax_global_options.showVideoInLightbox){
				showVideoLightbox.call(self, this.id);
			} else {
	...
	getPlaylists = function(nextPageToken) {
		...
		var apiChannelPlaylistsURL = "https://www.googleapis.com/youtube/v3/playlists?part=contentDetails,snippet&channelId="+youmaxChannelId+"&maxResults="+this.youmax_global_options.maxResults+pageTokenUrl+"&key="+this.youmax_global_options.apiKey;
		var self = this;
	...
	$.fn.youmax = function(options) {
		...
		this.youmax_global_options.showVideoInLightbox = options.showVideoInLightbox||false;
