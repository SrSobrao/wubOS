(function(){ 
	var reloj = new App('reloj', 'apps/reloj/img/favicon.png', 'Reloj',
					[], ['apps/reloj/index.css'], null, null, true);
	var digitalClock = function(config){
		...
		function Digit(bReal, iPos){
			...
			var eElem	=	(typeof oConfig.oObj === 'string' || oConfig.oObj instanceof String)?document.getElementById(oConfig.oObj):oConfig.oObj;
		...
		if(initDigits()){
			setNowTime();
			timer = setInterval(setNowTime, 1000);
		}
		return timer;
	};
	reloj.run = function(){
		...
		var prog = new Program(self, function(){
			...
			v.getDivContenido().className += " reloj";
			v.setIcono('apps/reloj/img/favicon.png');
			v.setTitulo('Reloj');
			v.onClose = function(){
			...
			var tTimer = null;
			v.cargarContenidoArchivo('apps/reloj/index.xml', function(){
				v.mostrar();
				tTimer = digitalClock({
					element			:	v.controles.baseReloj,
					iDigitWidth	:	40,
					iLineSize		:	8,
					iGap				:	2,
					sDigitColor	: '#00ff00'
	...
	reloj.widget = function(){
		var contenido = '<center id="widgetReloj" class="btnMetro"></center>';
		$(OS.dashboarddMenuContentGrid).AddMetroDoubleWithLabelContentString('btnMetro', contenido, "Reloj", 'OS.getAppByName("reloj").run()','metro-laranja', 'black');
		digitalClock({
			element			:	"widgetReloj",
			iDigitWidth	:	30,
			iLineSize		:	6,
