var ventana = function ventana(procesoParent ,opciones)
{
	...
	divBase.style.left = (0 + configuracion.posicionX) + "px";
	...
	this.cambiarEscritorio = function(num)
	{
		if(num >= 0 && num < OS.escritorios.length)
		{
			...
			{
				var diff = num - escAct;
				OS.escritorios[num].appendChild(ventAct.getDivBase());
				var esceso = 0;
				if(virtualX)
					esceso = Math.abs(this.getX() - virtualX) * Math.sign(diff);
				else
					virtualX = this.getX();
				this.setX(this.getX() - ((diff * window.innerWidth) - esceso));
				this.setX(virtualX, OS.efectosVentana.cambiarEscritorio,
					function(){
						virtualX = null;
					});
				//divBase
		...
		if(OS.efectosVentana.maximizar)
		{
			...
			.effect( "transfer", { to: "#" + this.btnBarra.id, className: "ui-effects-transfer" }, 250,
				function()
				{
					...
					this.style.top = (parseFloat(this.style.top) + (window.innerHieght * 2)) + "px";
	...
	$(divBase).draggable({
		...
		stop: function( event, ui ) {
			var x = ventAct.getX();
			var escAct = OS.obtenerEscritorioVentana(ventAct);
			if(x < 0 && escAct.pos > 0)
			{
				OS.escritorios[escAct.pos - 1].appendChild(ventAct.getDivBase());
				ventAct.setX(window.innerWidth + x);
			}
			else if(x > window.innerWidth && escAct.pos < OS.escritorios.length - 1)
			{
				OS.escritorios[escAct.pos + 1].appendChild(ventAct.getDivBase());
				ventAct.setX(x - window.innerWidth);
			}
		}
	});
